...
│   ├── local-user-authorizer.ts `LocalUserAuthorizer` class - `TerminalAuthorizer` for a session with UI and the single `permissions:ui_prompt` emit site; brackets the human wait and durable write with Herdr's best-effort blocked-state convention, resolves durable local choices through `PersistentApprovalService`, while forwarded asks remain once/session-only
