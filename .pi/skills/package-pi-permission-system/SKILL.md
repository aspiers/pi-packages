...
The bash `external_directory` gate only sees tokens that `classifyTokenAsPathCandidate` accepts, the broader `path` surface sees what `classifyTokenAsRuleCandidate` accepts, and a bare filename is promoted into both when the existence probe finds it on disk; token shape is judged **after** `$HOME`/`$PWD` expansion, whose vocabulary lives only in `access-intent/bash/shell-variable-expansion.ts` — do not add a `$HOME` branch to a classifier.
